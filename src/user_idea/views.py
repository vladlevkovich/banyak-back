...
class IdeaViewSet(viewsets.ModelViewSet):
    ...
    def search(self, request):
        stack = request.GET.get('search')
        speciality = request.GET.get('speciality')
        if stack == '' and speciality == '':
            return Response({'result': []}, status=status.HTTP_200_OK)
        if speciality and stack:
            ideas = Idea.objects.filter(stack__name=stack, specialization__name=speciality)
        elif speciality and not stack:
            ideas = Idea.objects.filter(specialization__name=speciality)
        elif not speciality and stack:
            ideas = Idea.objects.filter(stack__name=stack)
        page = self.paginate_queryset(ideas)
        if page:
            serializer = self.serializer_class(page, many=True)
            return self.get_paginated_response(serializer.data)
    ...
    @action(methods=['PUT'], detail=False, url_path='avatar-update/<uuid:avatar_id>')
    def update_avatar_idea(self, request, avatar_id):
        # avatar_id = kwargs.get('id')
        avatar_idea = AvatarIdea.objects.get(user=request.user, id=avatar_id)
        serializer = UpdateIdeaAvatarSerializer(avatar_idea, data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data, status=status.HTTP_200_OK)
