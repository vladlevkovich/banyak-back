...
class AvatarIdeaSerializer(serializers.ModelSerializer):
...
class IdeasListSerializer(serializers.ModelSerializer):
    ...
    avatar = AvatarIdeaSerializer(many=False)
...
class UpdateIdeaSpecialitySerializer(serializers.ModelSerializer):
    class Meta:
        model = Speciality
        field = ('name', 'is_find')
...
class UpdateCreateIdeaSerializer(serializers.ModelSerializer):
    ...
    # avatar = UpdateIdeaSpecialitySerializer(many=True)
    ...
    def create(self, validated_data):
        ...
        for specialization in specialization_list:
            ...
            if specialization_name:
                specialization_instance, _ = Speciality.objects.get_or_create(name=specialization_name.upper())
                idea.specialization.add(specialization_instance)

        for stack in stack_list:
            stack_name = stack.get('name')
            if stack_name:
                stack_instance, _ = Stack.objects.get_or_create(name=stack_name.upper())
...
class UpdateIdeaAvatarSerializer(serializers.ModelSerializer):
    class Meta:
        model = AvatarIdea
        fields = ('avatar', )
