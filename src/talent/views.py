...
class TalentsViews(viewsets.ModelViewSet):
    ...
    def search(self, request):
        stack = request.query_params.get('query')
        speciality = request.query_params.get('speciality')
        if stack == '' and speciality == '':
            return Response({'result': []}, status=status.HTTP_200_OK)
        elif stack and speciality:
            talents = UserProfile.objects.filter(stack__name=stack, speciality__name=speciality)
        elif stack and not speciality:
            talents = UserProfile.objects.filter(stack__name=stack)
        elif not stack and speciality:
            talents = UserProfile.objects.filter(speciality__name=speciality)
        page = self.paginate_queryset(talents)
        if page:
            pass
